...
module.exports = (function() {
	...
	Connection.prototype.post6_43_1LoginStateMachine = function post6_43_1LoginStateMachine(lc) {
		/**
	...
	Connection.prototype.loginStateMachine = function loginStateMachine() {
		...
		lc.on('trap', function loginTrap(data) {
			/** @this is now mikronode-ng.Channel */
			if (data.errors.length == 1 && data.errors.find(i => i.toString().includes('invalid user name or password'))) {
				console.error('post 6.43.1 login');
				_this.post6_43_1LoginStateMachine(lc);
				return;
			}
			_(_this).status = 'Received Trap';
			debugLogin('%s %o', _(_this).status, data);
